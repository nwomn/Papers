...
# 导入并注册MOB模型
try:
    sys.path.insert(0, '/9950backfile/srl/casual-mtl/models/src')
    
    from mob_gated_deltanet import MobGatedDeltaNetConfig, MobGatedDeltaNetForCausalLM
...
def process_single_config(hidden_size, head_dim, num_heads, num_hidden_layers):
    ...
    CUSTOM_CONFIG_PARAMS = {
        ...
        # --- MoB-GatedDeltaNet/FLA 特定参数 ---
        "attn_mode": "chunk",
        "aux_loss_scale": 0, # modified from 0.01
        "conv_size": 4,
        "expand_v": 1,
        "ratio": 3,
        "shared_head": 3,
    ...
    # =========================================================================
    print(f"[INFO] 正在创建MOB模型实例 (hidden_size={hidden_size}, num_layers={num_hidden_layers}, num_heads={num_heads}, head_dim={head_dim})...")
    
    # 即使某些参数不会被保存到最终的json，也需要传入Config类以正确构建模型
    model_config = MobGatedDeltaNetConfig(**CUSTOM_CONFIG_PARAMS)
    model = MobGatedDeltaNetForCausalLM(config=model_config)
    ...
    # 生成模型名称
    MODEL_NAME = f"{EXP_GROUP}-{hidden_size}-{num_hidden_layers}-{param_count_m}M"
    ...
    KEYS_TO_SAVE = [
        ...
        "head_dim", "hidden_act", "hidden_ratio", "aux_loss_scale",
        "hidden_size", "initializer_range", "intermediate_size", 
        "max_position_embeddings", "model_type", "norm_eps", "fuse_norm", "fuse_swiglu",
        "num_heads", "num_hidden_layers", "ratio", "shared_head", 
        "topk", "num_block", "overlap", "policy", "use_layer_wise_balance",
...
def main():
    ...
    """
    print(f"[INFO] 开始批量生成MOB模型配置，共有 {len(CONFIG_COMBINATIONS)} 种配置组合")
    print("=" * 80)
