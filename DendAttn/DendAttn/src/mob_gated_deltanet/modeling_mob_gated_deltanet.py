...
from .layer import MobGatedDeltaNet
from .configuration_mob_gated_deltanet import MobGatedDeltaNetConfig
from fla.models.utils import Cache
from fla.modules import FusedCrossEntropyLoss, FusedLinearCrossEntropyLoss
from fla.modules import GatedMLP as MobGatedDeltaNetMLP
...
class MobGatedDeltaNetBlock(nn.Module):
    def __init__(self, config: MobGatedDeltaNetConfig, layer_idx: int):
        super().__init__()
...
class MobGatedDeltaNetPreTrainedModel(PreTrainedModel):

    config_class = MobGatedDeltaNetConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ['MobGatedDeltaNetBlock']
...
@dataclass
class MobGatedDeltaNetOutputWithPast(BaseModelOutputWithPast):
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MobGatedDeltaNetModel(MobGatedDeltaNetPreTrainedModel):

    def __init__(self, config: MobGatedDeltaNetConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embeddings = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([MobGatedDeltaNetBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        return MobGatedDeltaNetOutputWithPast(
...
@dataclass
class MobGatedDeltaNetCausalLMOutputWithPast(CausalLMOutputWithPast):
    aux_loss: Optional[torch.FloatTensor] = None
    router_logits: Optional[Tuple[torch.FloatTensor, ...]] = None

class MobGatedDeltaNetForCausalLM(MobGatedDeltaNetPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MobGatedDeltaNetModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        return MobGatedDeltaNetCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
def load_balancing_loss_func(
    ...
    all_balance_losses = []
    B, S, E = gate_logits[0].shape
    assert num_experts == E, "num_experts must match ratio-shared_head"

    for logits in gate_logits:
        # logits: [B, S, E]
        flat_logits = logits.view(-1, E)          # [B*S, E]
        probs = flat_logits.softmax(dim=-1)
        _, selected_experts = torch.topk(probs, top_k, dim=-1)  # [B*S, k]
        
        # if selected_experts.dtype != torch.int64:
        #     selected_experts = selected_experts.to(torch.int64)
        # if len(selected_experts.shape) == 1:
        #     selected_experts = selected_experts.unsqueeze(1)
            
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=E)  # [B*S, k, E]
        expert_mask = torch.max(expert_mask, dim=-2).values.float()  # [B*S, E]

        # 全局统计
        f_i = expert_mask.mean(0)  # [E]
        P_i = probs.mean(0)  # [E]

        balance_loss = (f_i * P_i).sum() * E
        all_balance_losses.append(balance_loss)

    all_balance_losses = torch.stack(all_balance_losses).mean() 
