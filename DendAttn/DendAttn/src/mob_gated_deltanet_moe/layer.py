...
class MobGatedDeltaNetMoE(nn.Module):
    """
    ...
    def sparse(self,hidden_states):
        ...
        router_logits = self.gate(hidden_states)  # (hb, l, ratio)
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # 稀疏路由 (Triton 优化版本)
        from ops.sparse_routing import sparse_routing_triton
        router_mask, router_weight_full = sparse_routing_triton(
            q, self.gate, self.num_heads, self.ratio, self.shared_head, self.topk
        )
        # 原实现:
